...
figure;
subplot(1, 3, 1);
imshow(input_s); title('Original Image for Step 1');

subplot(1, 3, 2);
imshow(gaussian_noisy_image); title('Gaussian Noisy Image');

subplot(1, 3, 3);
imshow(denoised_gaussian); title(['Denoised Gaussian Image, PSNR: ', num2str(psnr_value_gaussian, '%.2f'), ' dB']);
...
subplot(1, 3, 2);
imshow(impulse_noisy_image); title('Salt & Pepper Noisy Image');

subplot(1, 3, 3);
imshow(denoised_impulse); title(['Denoised Salt & Pepper Image, PSNR: ', num2str(psnr_value_impulse, '%.2f'), ' dB']);
...
% Display (5) Frequency Domain After Notch Filtering (Visualize Red Channel Only)
...
% Display (6) Restored Image in Spatial Domain
figure;
imshow(restored_s, []);
title('Restored Image in Spatial Domain after Notch Filtering');
...
for k = 1:length(K_values_3)
    ...
    for c = 1:3
        ...
        denoised_gaussian = denoiseWithGaussian(noisy_blurred_image(:, :, c), 1.5);
...
title('Noisy Motion Blurred Image');
...
motionLength = 20; % Estimated length of the motion blur
motionAngle = 5; % Estimated angle of the motion blur
...
numIterations = [20, 30, 40]; % Number of iterations
